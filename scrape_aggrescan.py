...
def parse_na4vss(text: str, sequence_name: str | None = None) -> str:
    ...
    if sorted_match:
        ...
        if sequence_name:
            summary_patterns.append(
                rf"^\s*{escaped_name}\b\s+([-+]?\d+(?:\.\d+)?(?:[eE][-+]?\d+)?)(?:\s+.*)?$"
            )
        summary_patterns.append(
            r"^\s*[A-Za-z0-9]{4}(?:[_:-][A-Za-z0-9]+)?\b\s+([-+]?\d+(?:\.\d+)?(?:[eE][-+]?\d+)?)(?:\s+.*)?$"
        )
    ...
    if sequence_name:
        escaped_name = re.escape(sequence_name.strip())
        metrics_match = re.search(
            rf"^\s*{escaped_name}\s*$"
            r"(?P<metrics>.*?)(?:^\s*#\s+AA\b|^\s*Average\s+over\s+all\s+sequences\b|\Z)",
            text,
        ...
        if metrics_match:
            metric_values = re.findall(
                r"^\s*([-+]?\d+(?:\.\d+)?(?:[eE][-+]?\d+)?)\s*$",
                metrics_match.group("metrics"),
                re.MULTILINE,
            )
            if len(metric_values) >= 9:
                return metric_values[8].strip()
